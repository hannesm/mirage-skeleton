...
let domain = Domain_name.of_string_exn "mirage"

let hostname = Domain_name.prepend_label_exn domain "charrua"

let reserved_host_names =
  List.fold_right
    Domain_name.Set.add
    (List.map (Domain_name.prepend_label_exn domain)
...
let options = [
  ...
  Domain_name (Domain_name.to_string domain);
  (*
